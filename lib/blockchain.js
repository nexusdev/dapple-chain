...
var DappleChain = function(opts, cb) {
  ...
  console.log("DC consturctor pre init");
  // async.waterfall([
  ...
  //     console.log("DC constructor pre Blockchain init");
  //     this.blockchain = new Blockchain(this.db, false);
  //     console.log("DC constructor post Blockchain init");
  //     cb();
  //   }).bind(this),
  //   (cb) => {
  //     console.log("DC trivial call after Blockchain init");
  //     cb();
  //   }
...
DappleChain.prototype.addAccount = function(opts, callback) {
  ...
  var secretKey = opts.secretKey ? Buffer(toHex(opts.secretKey).slice(2), 'hex') : crypto.randomBytes(32);
...
DappleChain.prototype.setBalance = function (address, balance, cb) {
  var that = this;
  address = Buffer(toHex(address).slice(2), 'hex');
  balance = Buffer(toHex(balance).slice(2), 'hex');
...
DappleChain.prototype.getTransactionCount = function(address, callback) {
  ...
  this.vm.stateManager.getAccount(address, function(err, result) {
    if (err != null) {
      ...
      callback(null, toHex(result.nonce));
    }
  });
}
