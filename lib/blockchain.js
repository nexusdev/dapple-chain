...
var Blockchain_ = function(opts) {
  if(opts.packageRoot) { // if working dir is set, work with persistant db
    ...
    // try {
      this.state = deasync(this.db.get).apply(this.db, ["state", {valueEncoding: 'json'}]);
  ...
  var that = this;
  // Setup geth db
  // get latest block from geth leveldb
  /*
  var db2 = levelup('/Users/mhhf/Library/Ethereum/chaindata');
  db2.get(new Buffer('LastBlock'), {valueEncoding: 'hex'}, (err, blockHash) => {
    db2.get(new Buffer('626c6f636b2d' + blockHash+'2d686561646572', 'hex'), {valueEncoding: 'hex' },(err, block) => {
      let header = rlp.decode(new Buffer(block, 'hex'));
      let b = new Block([header, [], []]);
      that.state.pointers.livenet = {
        branch: true,
  ...
  */
  ...
  this.stateTrie.get(new Buffer('acc46a2a555c74ded4a2bd094e821b97843b40c0', 'hex'), (err, res) => {
    console.log(err, res);
  });

  // this.stateTrie._getDBs.push();
  //
  //
  // this.db.get(new Buffer('d7f8974fb5ac78d9ac099b9ad5018bedc2ce0a72dad1827a1709da30580f0544','hex'), (err, res) => {
  //   console.log(rlp.decode(res));
  // })


  // this.stateTrie._walkTrie(this.state.pointers[this.state.head].stateRoot, function(a,b,c,walkController) {
  //   console.log(a,b,c);
  //    walkController.next();
  // }, function() {
  //   console.log('---');
  ...
  // this.stateTrie.getRaw('0x440dc3c8dd8800e16466d4c3c64162a647c4484c01601a5ef59d74e58fa80d1d', function(err,res){
  //   console.log(rlp.decode(res));
  // });

  // TODO - learn how to copy a raw database
  // 1. write a rest api which exports a rawOutput of the root state trie
  // 2. import this root state trie (fork root)
  // 3. work from this state root on
  //
...
Blockchain_.prototype.updateState = function () {
...
Blockchain_.prototype.loadState = function (state) {
  var env = state.pointers[state.head];
  this.stateTrie.root = new Buffer(env.stateRoot.slice(2), 'hex');
  deasync(this.db.put).apply(this.db, ['meta', env.meta, {valueEncoding: 'json'}]);
}

Blockchain_.prototype._saveState = function () {
...
Blockchain_.prototype.fork = function (name) {
...
Blockchain_.prototype.checkout = function (name) {
