...
var DappleChain = function(opts) {
  ...
  // TODO - refactor this mess
    async.parallel([
      ...
      this.fakedOwnership = []; // list of addresses from which transactions should be accepted without a privateKey given
...
DappleChain.prototype.fakedOwnership = function (address) {
  this.fakedOwnership.push(address);
}
...
DappleChain.prototype.processTransaction = function(from, rawTx, callback) {
  ...
  if( this.fakedOwnership.indexOf(from) == -1 ) {
  ...
  this.stateTrie.get(address, function(err, val) {
    ...
    if( self.fakedOwnership.indexOf(from) == -1 ) {
      tx.sign(privateKey);
    } else {
