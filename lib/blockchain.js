...
// TODO - why does the geth chain dont geth intitialized?
...
var Blockchain_ = function(opts) {
  ...
  if(opts.packageRoot) { // if working dir is set, work with persistant db
    ...
    async.parallel([
      levelup.bind(this, path.join(opts.packageRoot,'.dapple/chain_db')),
      (cb) => { let gi = new GethInterface('/Users/mhhf/Library/Ethereum/chaindata', () => { cb(null, gi) });}
    ], (err, res) => {
      this.db = res[0]
      this.gethInterface = res[1];
      this.gethInterface.initDb(this.db);
      ...
      async.waterfall([
        ...
        this.gethInterface.initLazyLoading();
        this._initDone = true;
      })
    });

    // this.db.get('557e59513042f04134c7311bdef3026940060d935ab0fc3a1f57fdd253fbcc70', {valueEncoding: 'hex'}, (err, res) => {
    //   console.log(rlp.decode(new Buffer(res,'hex')));
    // });
...
Blockchain_.prototype.initState = function (cb) {
...
// TODO - better name
Blockchain_.prototype.loadState = function (cb) {
  var env = this.state.pointers[this.state.head];
...
Blockchain_.prototype.createState = function (callback) {
  var genesis = {
  ...
  this.gethInterface.forkLatest((err, env) => {
...
Blockchain_.prototype.restoreOldState = function (cb) {
